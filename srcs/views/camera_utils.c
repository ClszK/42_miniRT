...
/*   Updated: 2024/05/02 21:05:48 by ljh              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	camera_init(t_canvas cvs, t_camera *cam)
{
	...
	fov_radians = degrees_to_radians(cam->fov);
	cvs.focal_length = cvs.viewport_width / (2.0 * tan(fov_radians / 2.0));
	cam->right = vec3_unit(vec3_cross(&cvs.world_up, &cam->cam_dir));
	up = vec3_cross(&cam->cam_dir, &cam->right);
